...
bool ImGui::Combo(const char* label, int* current_item_index, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ImGuiContext& g = *GImGui;

    // Call the getter to obtain the preview string which is a parameter to BeginCombo()
    const char* preview_value = NULL;
    if (*current_item_index >= 0 && *current_item_index < items_count)
        items_getter(data, *current_item_index, &preview_value);
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        const bool item_selected = (i == *current_item_index);
...
bool ImGui::Combo(const char* label, int* current_item_index, const char* const items[], int items_count, int height_in_items)
{
    const bool value_changed = Combo(label, current_item_index, Items_ArrayGetter, (void*)items, items_count, height_in_items);
    return value_changed;
}

// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items "item1\0item2\0"
bool ImGui::Combo(const char* label, int* current_item_index, const char* items_separated_by_zeros, int height_in_items)
{
    int items_count = 0;
    ...
    bool value_changed = Combo(label, current_item_index, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}
...
bool ImGui::ListBox(const char* label, int* current_item_index, const char* const items[], int items_count, int height_items)
{
    const bool value_changed = ListBox(label, current_item_index, Items_ArrayGetter, (void*)items, items_count, height_items);
    return value_changed;
}

// This is merely a helper around BeginListBox(), EndListBox().
// Considering using those directly to submit custom data or store selection differently.
bool ImGui::ListBox(const char* label, int* current_item_index, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ImGuiContext& g = *GImGui;
